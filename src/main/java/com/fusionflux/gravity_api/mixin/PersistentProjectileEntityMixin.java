...
//                    target = "Lnet/minecraft/entity/projectile/PersistentProjectileEntity;<init>(Lnet/minecraft/entity/EntityType;DDDLnet/minecraft/world/World;)V"
    ...
    private static void modifyargs_init_init_0(Args args) {
        Direction gravityDirection = GravityChangerAPI.getGravityDirection(args.get(1));
        if(gravityDirection == Direction.DOWN) return;

        Vec3d pos = ((LivingEntity) args.get(1)).getEyePos().subtract(RotationUtil.vecPlayerToWorld(0.0D, 0.10000000149011612D, 0.0D, gravityDirection));
        args.set(1, pos.x);
        args.set(2, pos.y);
        args.set(3, pos.z);
    }
